...
|---|---|---|
| [`inventory/SKILL.md`](inventory/SKILL.md) | **Answer "what's available?" ONLY from ground truth** — runs `claw robots/templates/scenes/envs` (or reads `inventory/INVENTORY.md`); never invent robots/scenes, never list physical-arm skills as sim robots | available, what robots, what environments, what scenes, list robots, list tasks, catalog, inventory, what can I open, what can I train |

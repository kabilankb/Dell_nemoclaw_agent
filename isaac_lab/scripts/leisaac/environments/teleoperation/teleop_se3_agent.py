...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="ZMQ endpoint for OpenClaw device to listen on",
    )
    ...
    parser.add_argument("--enable_vision", action="store_true", help="Enable YOLO vision publisher for NemoClaw.")
    parser.add_argument("--yolo_model", type=str, default="yolo11n.pt", help="YOLO model path for vision detection.")
    parser.add_argument("--vision_port", type=int, default=5559, help="ZMQ port for vision detection publisher.")
    parser.add_argument("--vision_interval", type=int, default=15, help="Run YOLO detection every N simulation frames.")
    ...
    def main():  # noqa: C901
        ...
        teleop_device_for_cfg = "keyboard" if args_cli.teleop_device == "openclaw" else args_cli.teleop_device
        env_cfg.use_teleop_device(teleop_device_for_cfg)
        ...
        elif args_cli.teleop_device == "openclaw":
            from leisaac.devices import OpenClawDevice

            teleop_interface = OpenClawDevice(env, endpoint=args_cli.openclaw_endpoint, step_hz=args_cli.step_hz)
        else:
            raise ValueError(
        ...
        if args_cli.enable_vision:
            from leisaac.devices.openclaw.vision_publisher import VisionPublisher

            vision_publisher = VisionPublisher(
                env,
                yolo_model=args_cli.yolo_model,
                port=args_cli.vision_port,
                detect_interval=args_cli.vision_interval,
            )
            ...
            while simulation_app.is_running() and not interrupted:
                ...
                with torch.inference_mode():
                    ...
                    # apply actions
                        ...
                        env.step(actions)
                    if vision_publisher is not None:
                        vision_publisher.update()
